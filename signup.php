        ...
        if(!empty($form['pic_name'])){
            $_SESSION['kakunin']['pic_name']=$form['pic_name'];
              ...
              if(!empty($_FILES['pic']['name'])){
                $dir='/usr/local/www/a1.zeroprm.com/htdocs/b31_c577/keijiban/profile_img/';
...
function errorCheck($form){
    ...
    if(isset($form['email'])){
        $sql=sprintf('SELECT COUNT(*) AS cnt FROM user_profile WHERE user_email="%s"',mysql_real_escape_string($form['email']));
        $recode = mysql_query($sql) or die(mysql_error());//mysql_query() は、 ひとつのクエリを送信します
        $table = mysql_fetch_assoc($recode);//mysql_fetch_assoc — 連想配列として結果の行を取得する
        if ($table['cnt']>0) {
                            ...
                            <div class="span contact-form centered">
                                ...
                                <form id="contact-form" action="" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="control-group">
                                        <div class="controls">
                                            <p>メールアドレス</p>
                                            <input type="text" name="email" class="span10" placeholder="メールアドレス" value="<?php echo htmlspecialchars($form['email'])?>" maxlenth="255" />
                                            <!--エラー警告表示-->
                                            <?php if (!empty($error['email']) and $error['email']=="blank") {echo "メールアドレスを入力してください";}
                                            if (!empty($error['email']) and $error['email']=="notemail") {echo "メールアドレスの形式で入力してください";}
                                            if (!empty($error['email']) and $error['email']=="duplicate") {echo "同じメールアドレスで登録されています";}
                                            ?>
                                        </div>
                                    </div>
                                    ...
                                    <div class="control-group">
                                        <div class="controls">
                                                 <p>パスワード</p>
                                                <input type="password" name="password" class="span10" placeholder="パスワード" value="<?php echo htmlspecialchars($form['password']) ?>" maxlenth="255" />
                                                <!--エラー警告表示-->
                                                <?php if (!empty($error['password']) and $error['password']=="blank") {echo "パスワードを入力してください";}
                                                if (!empty($error['password']) and $error['password']=="under") {echo "パスワードを８文字以上入力してください";}
                                                ?>
                                        </div>
                                    </div>
